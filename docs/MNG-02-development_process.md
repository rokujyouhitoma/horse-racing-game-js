...
### 2026年6月
* **2026.6.14**: Closure Compiler のアップグレードに伴うコンパイル警告およびエラーの修正（ADR-04）。機能的バグ（`replace` 冗長引数、ジェネレータ反復型指定のミスマッチ、`Locator` のキー安全性向上）を修正し、過剰なスタイルチェックフラグを Makefile から除去。
