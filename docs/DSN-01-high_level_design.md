# [DSN-01] 基本設計書 (High-Level Design) - horse-racing-game-js

本ドキュメントは、バニラJavaScriptで構築されたブラウザベースの競馬ボードゲーム「`horse-racing-game-js`」の全体構成および論理的なシステム設計を定義したものです。

---

## 1. システム概要とビジネスバリュー

本プロジェクトは、カードとオッズを駆使して競う戦略性の高い「競馬ボードゲーム」をデジタル化し、ウェブブラウザで手軽にプレイできる環境を提供します。
...
## 3. UI/UX 設計およびアクセシビリティ方針 (UI/UX Design Policies)

ゲーム本来の楽しさに集中できるよう、ユーザーインターフェース（UI）およびユーザー体験（UX）において以下の方針を最優先します。

* **直感的な操作の重視**:
  * ユーザーがマニュアルや説明を読むことなく、Webブラウザ上で直感的な操作で使えることを最優先の設計・開発方針とします。
* **自己記述的なUIデザイン**:
  * 各種ボタンや操作領域は、配置やアイコンデザイン自体がその機能や操作方向を視覚的に自己説明するものとします。

---

## 4. 全体システムアーキテクチャ (High-Level Architecture)

システムは、機能ごとに明確に役割を分離した「疎結合（独立性が高い）設計」を採用しています。これにより、一部分の変更が全体に悪影響を及ぼさない安定したシステムを実現しています。

```mermaid
graph LR
    subgraph UI ["ユーザーインターフェース (見た目)"]
        UI_Layer["画面表示・操作レイヤー"]
        TE["テンプレートエンジン"]
    end
    
    subgraph Core ["コア・ゲームロジック (仕組み)"]
        GE["ゲームエンジン"]
        Rules["ゲームルール・判定"]
        DM["データ管理・保存"]
    end

    UI_Layer <-->|Pub/Sub イベント通信| GE
    TE --> UI_Layer
    GE --> Rules
    GE --> DM
```
