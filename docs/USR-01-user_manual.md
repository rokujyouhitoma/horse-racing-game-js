...
本ゲームは**直感的なUI/UX**を最優先に設計されており、マニュアルを読まなくても直感的に操作できる**自己記述的なUIデザイン**（自己説明的なボタン配置やビジュアル表現）を採用しています。
...
2. **レース画面**:
   ...
   * 画面右下に山札と手札が表示されます。
   * 画面上部には、カードをランダムに1枚プレイする **「Play PlayCard Random」** ボタン、ゲーム全体をリセットする **「Reset 🎮」** ボタンが配置されています。
   * 画面右上には、トグルメニューを開くための **「≡」** ボタンが設置されています。メニュー内の **「Debug」** を選択することで、開発・検証用の **「Debug Menu（デバッグメニュー）」** の表示・非表示を切り替えることができます。
   * **デバッグメニュー (Debug Menu) の機能一覧**:
     * **Auto PlayCard**: 有効にすると、カードのドローとプレイが自動で繰り返され、レースが自動進行します。
     * **Reset 🎮**: ゲーム全体を初期化（タイトル画面への遷移等）します。
     * **Reset 🏇**: レース盤面とカードを初期化します。
     * **Play PlayCard**: 山札から手札へカードを引き、ランダムにプレイします。
     * **Play Undo PlayCard**: 直前にプレイされたカードの効果を1手巻き戻します (Undo)。
     * **Play RankCard**: 順位カード（RankCard）を強制プレイします。
     * **Play DashCard**: ダッシュカード（DashCard）を強制プレイします。
     * **Check Relationship**: マスターデータの整合性チェックをコンソール上で実行します。
     * **seed (シード入力ボックス) & Set & Reset 🏇**: 任意のシード値を入力し、このボタンを押すことで、指定されたシード値を適用してレース盤面とカード山札をリセットします。過去に遊んだ同じシード値を指定してこのボタンを押すことで、カード山札のシャッフル順序を含むすべてのゲーム展開を完全に再現することが可能です。
3. **結果表示画面**:
